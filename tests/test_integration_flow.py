...
def test_session_upload_profile_facts_spec_report_flow() -> None:
    session_resp = client.post("/sessions", json={"created_by": ACTOR}, headers={"X-API-Key": ACTOR})
    ...
    profile_resp = client.get(f"/sessions/{dataset_id}/profile", headers={"X-API-Key": ACTOR})
    assert profile_resp.status_code == 200
    assert "profile" in profile_resp.json()

    facts_job_resp = client.post(f"/sessions/{dataset_id}/facts", params={"mode": "sample"}, headers={"X-API-Key": ACTOR})
    ...
    facts_status = client.get(f"/jobs/{facts_job_id}", headers={"X-API-Key": ACTOR})
    assert facts_status.status_code == 200
    assert facts_status.json()["status"] == "succeeded"

    facts_resp = client.get(f"/sessions/{dataset_id}/facts", headers={"X-API-Key": ACTOR})
    ...
    report_resp = client.post(f"/sessions/{dataset_id}/report", json={}, headers={"X-API-Key": ACTOR})
    ...
    report_status = client.get(f"/jobs/{report_job_id}", headers={"X-API-Key": ACTOR})
    assert report_status.status_code == 200
    assert report_status.json()["status"] == "succeeded"

    pdf_resp = client.get(f"/sessions/{dataset_id}/export/pdf", headers={"X-API-Key": ACTOR})
    assert pdf_resp.status_code == 200
    assert pdf_resp.headers.get("content-type", "").startswith("application/pdf")

    audit_resp = client.get(f"/sessions/{dataset_id}/audit", headers={"X-API-Key": ACTOR})
